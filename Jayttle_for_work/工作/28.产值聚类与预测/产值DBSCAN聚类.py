...
def 读取文件夹中的所有Excel文件(folder_path: str) -> List[pd.DataFrame]:
    """
    ...
    for file in excel_files:
        ...
        if '现价产值' in df.columns:
            df_filtered = df_filtered[['现价产值', df.columns[0]]]  # 第一列和"现价产值"列
            
            # 调整列的顺序，使得第一列在前
            df_filtered = df_filtered[[df.columns[0], '现价产值']]
...
def 聚类分析(df, eps=0.5, min_samples=5):
    """
    使用 DBSCAN 聚类算法进行聚类分析。
    
    :param df: 输入数据框
    :param eps: DBSCAN 的邻域大小，控制聚类的密度
    :param min_samples: 核心点所需的最小邻居数
    :return: 聚类后的 DataFrame 和 DBSCAN 模型
    """
    # 数据标准化
    ...
    labels = dbscan.fit_predict(standardized_df)  # 使用标准化后的数据进行聚类
    
    # 将聚类结果添加到 DataFrame 中
    standardized_df['Cluster'] = labels
    
    return standardized_df, dbscan

def 可视化聚类结果(final_df, dbscan, labels):
    """
    可视化 DBSCAN 聚类结果。
    
    :param final_df: 原始的 DataFrame
    :param dbscan: DBSCAN 聚类模型
    ...
    plt.xlabel('时间')
    plt.ylabel('现价产值')
    plt.legend(title='Cluster', labels=[f'Cluster {i+1}' for i in range(len(np.unique(labels)))])
    plt.show()
...
if __name__ == '__main__':
    ...
    绘制散点图和折线图(final_df)
    # 聚类分析
    final_df_standardized, kmeans = 聚类分析(final_df)  # 假设聚为2类
