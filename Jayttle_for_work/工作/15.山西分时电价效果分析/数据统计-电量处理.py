...
def run_main_2021(file_path: str, isWinter: bool = False) -> None:
    # find_eva_date()
    sum_results_GF = df_monthly_analysis_GF(file_path, 2021, isWinter)
    sum_results_NoGF = df_monthly_analysis_NOGF(file_path, 2021, isWinter)
    sum_results_GF['峰'] += sum_results_NoGF['峰']
    sum_results_GF['谷'] += sum_results_NoGF['谷']
    sum_results_GF['平'] += sum_results_NoGF['平']
    # 计算总和
...
def run_main_2024(file_path: str,isWinter: bool = False) -> None:
    # find_eva_date()
    sum_results_GF = df_monthly_analysis_GF(file_path, 2024, isWinter)
    sum_results_NoGF = df_monthly_analysis_NOGF(file_path, 2024, isWinter)
    sum_results_GF['峰'] += sum_results_NoGF['峰']
    sum_results_GF['谷'] += sum_results_NoGF['谷']
    sum_results_GF['平'] += sum_results_NoGF['平']
...
def run_main():
    ...
    for item in marker_name:
        ...
        data_row = {
            '分类': item,
            '高峰电量_2024': sum_results_2024['高峰'],
            '峰电量_2024': sum_results_2024['峰'],
            '平电量_2024': sum_results_2024['平'],
            '谷电量_2024': sum_results_2024['谷'],
            '高峰占比_2024': percen_2024['高峰'],
            '峰占比_2024': percen_2024['峰'],
            '平占比_2024': percen_2024['平'],
            '谷占比_2024': percen_2024['谷'],
            '高峰占比提升%': compare_percent['高峰'],
            '峰占比提升%': compare_percent['峰'],
            '平占比提升%': compare_percent['平'],
            '谷占比提升%': compare_percent['谷'],
            '整体用电量提升%': compare_percent['总和']
        }
        print(rf"{item}用户尖峰、峰、平、谷电量分别为{sum_results_2024['高峰']}、{sum_results_2024['峰']}、{sum_results_2024['平']}、{sum_results_2024['谷']}亿千瓦时，占比分别为{percen_2024['高峰']}%、{percen_2024['峰']}%、{percen_2024['平']}%、{percen_2024['谷']}%，较分时电价调整前（2021年6-8月）提高{compare_percent['高峰']}%、{compare_percent['峰']}%、{compare_percent['平']}%、{compare_percent['谷']}%。整体用电量了{compare_percent['总和']}%")
        # 将该行数据添加到结果列表中
    ...
    # 保存到Excel文件
    output_file = rf"C:\Users\juntaox\Desktop\夏季典型日电量分析结果.xlsx"
...
def run_main1():
    ...
    for item in marker_name:
        print(f"--------------------{item}-----------------------")
        file_path = rf"C:\Users\juntaox\Desktop\工作\15.山西分时电价成效分析\数据表\电量表-{item}.xlsx"
        sum_results_2021, percen_2021 = run_main_2021(file_path, isWinter=True)
        sum_results_2024, percen_2024 = run_main_2024(file_path, isWinter=True)
        # 计算比较百分比
        compare_percent = {key: round(percen_2024[key] - percen_2021[key], 6) for key in percen_2021.keys()}

        compare_percent['总体'] = round((percen_2024['总和'] - percen_2021['总和']) / percen_2021['总和'] * 100, 4)

        GFcompare_ratio = round(compare_percent['高峰'] + compare_percent['峰'], 2)
        Gucompare_ratio = compare_percent['谷']
        GF_change = round(sum_results_2024['高峰'] + sum_results_2024['峰'] - sum_results_2021['高峰'] - sum_results_2021['峰'], 2)
        Gu_change = round(sum_results_2024['谷'] - sum_results_2021['谷'], 2)
        print(rf"电价高峰时段电量{GF_change}亿千瓦时，占比{GFcompare_ratio}%，低谷时段电量{Gu_change}亿千瓦时，占比增加{Gucompare_ratio}%")
