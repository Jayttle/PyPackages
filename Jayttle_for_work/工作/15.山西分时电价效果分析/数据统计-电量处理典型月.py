...
def df_monthly_analysis_GF(file_path: str, year: int = None, isWinter: bool = False):
    ...
    if not isWinter:
    # 筛选出2024年6月的数据
        filtered_df = df[(df['日期'].dt.year == year) & (df['日期'].dt.month.isin([7, 8]))]
    else:
        filtered_df = df[
            (df['日期'].dt.year == year) & (df['日期'].dt.month.isin([1])) | 
...
def df_monthly_analysis_NOGF(file_path: str, year: int = None, isWinter: bool = False):
    ...
    if not isWinter:
        filtered_df = df[(df['日期'].dt.year == year) & (df['日期'].dt.month.isin([6]))]
    else:
        filtered_df = df[
...
def df_monthly_analysis_GF_in_specify_month(file_path: str, year: int = None, specify_month: int = None):
    df = read_data(file_path)
    ...
    year = year if specify_month != 12 else year - 1
    filtered_df = df[(df['日期'].dt.year == year) & (df['日期'].dt.month.isin([specify_month]))]
    t_columns = [col for col in filtered_df.columns if col.startswith('T')]
    GFtime_ranges = {
        '峰': [(800, 1100), (1700, 1800), (2000, 2300)],
        '高峰': [(1800, 2000)],
...
def df_monthly_analysis_NOGF_in_specify_month(file_path: str, year: int = None, specify_month: int = None):
    df = read_data(file_path)
...
def run_main_2021(file_path: str, specify_month: int = None) -> None:
    # find_eva_date()
    sum_results_GF = df_monthly_analysis_GF_in_specify_month(file_path, 2021, specify_month) if specify_month in [1, 7 ,8, 12] else df_monthly_analysis_NOGF_in_specify_month(file_path, 2021, specify_month)
    # 计算总和
    if '高峰' in sum_results_GF:
    ...
    total_sum = sum(sum_results_GF.values())
...
def run_main_2024(file_path: str,specify_month: int = None) -> None:
    # find_eva_date()
    sum_results_GF = df_monthly_analysis_GF_in_specify_month(file_path, 2024, specify_month) if specify_month in [1, 7 ,8, 12] else df_monthly_analysis_NOGF_in_specify_month(file_path, 2024, specify_month)
...
def run_main() -> list[dict]:
    ...
    marker_month = [5]
    result_df = []  # 用于存储所有print的输出
    
    for item in marker_name:
        results_list = []
        result_df.append([f"--------------------{item}-----------------------"])  # 记录当前项
        file_path = rf"C:\Users\juntaox\Desktop\工作\15.山西分时电价成效分析\数据表\电量表-{item}.xlsx"
        for month in marker_month:
            percen_2021 = run_main_2021(file_path, month)
            sum_results_2024, percen_2024 = run_main_2024(file_path, month)
            # 计算比较百分比
            ...
            results_dict = {
                'Key': f'{item}_{month}',
                '尖峰_2024电量': sum_results_2024['高峰'] if '高峰' in sum_results_2024 else 'None',
                '峰_2024电量': sum_results_2024['峰'],
                '平_2024电量': sum_results_2024['平'],
                '谷_2024电量': sum_results_2024['谷'],
                '总电量': percen_2024['总和'],
                '总电量变化比例': compare_percent['总体'],
                '尖峰_2024占比': percen_2024['高峰'] if '高峰' in percen_2024 else 'None',
                '峰_2024占比': percen_2024['峰'],
                '平_2024占比': percen_2024['平'],
                '谷_2024占比': percen_2024['谷'],
                '尖峰占比变化': compare_percent['高峰'] if '高峰' in compare_percent else 'None',
                '峰占比变化': compare_percent['峰'],
                '平占比变化': compare_percent['平'],
                '谷占比变化': compare_percent['谷'],
            }
            results_list.append(results_dict)
            
            # 将结果打印内容保存到 result_df
            result_df.append([
                f"{item}用户峰、平、谷电量分别为{sum_results_2024['峰']}、{sum_results_2024['平']}、{sum_results_2024['谷']}亿千瓦时，"
                f"占比分别为{percen_2024['峰']}%、{percen_2024['平']}%、{percen_2024['谷']}%，"
                f"较分时电价调整前（2021年5月）提高{compare_percent['峰']}%、{compare_percent['平']}%、{compare_percent['谷']}%。"
                f"整体用电量变化了{compare_percent['总和']}%。"
            ])
        
        output_file = rf"C:\Users\juntaox\Desktop\工作\15.山西分时电价成效分析\{item}月数据电量分析结果.xlsx"
        df = pd.DataFrame(results_list)
        df.to_excel(output_file)
        
        # 保存结果输出到Excel
        result_df_output_file = rf"C:\Users\juntaox\Desktop\工作\15.山西分时电价成效分析\{item}_result_output.xlsx"
        result_df_df = pd.DataFrame(result_df, columns=["Result"])
        result_df_df.to_excel(result_df_output_file, index=False)
