...
def df_monthly_analysis_GF(file_path: str, year: int = None, isWinter: bool = False):
    ...
    if not isWinter:
    # 筛选出2024年6月的数据
        filtered_df = df[(df['日期'].dt.year == year) & (df['日期'].dt.month.isin([7]))]
    else:
        filtered_df = df[
...
def run_main_2021(file_path: str, isWinter: bool = False) -> None:
    ...
    sum_results_GF['峰'] = round(sum_results_GF['峰'] / 10000 ,2)
    sum_results_GF['谷'] = round(sum_results_GF['谷'] / 10000 ,2)
    sum_results_GF['平'] = round(sum_results_GF['平'] / 10000,2)
    sum_results_GF['高峰'] = round(sum_results_GF['高峰'] / 10000 ,2)
...
def run_main1():
    ...
    for item in marker_name:
        ...
        # 计算比较百分比
        compare_percent = {key: round(percen_2024[key] - percen_2021[key], 4) for key in percen_2021.keys()}
        ...
        Gucompare_ratio = round(compare_percent['谷'], 2)
        ...
        percent = round(percen_2024['峰'] + percen_2024['高峰'], 2)

        isImproveStr_GF = '提高' if GFcompare_ratio > 0 else '降低'
        isImproveStr_Gu = '提高' if Gucompare_ratio > 0 else '降低'
        print(rf"典型日电价高峰时段电量为{GF_change}万千瓦时,占全时段电量的{percent}%，较基准日的高峰时段占比{isImproveStr_GF}{GFcompare_ratio}%，低谷时段电量为{Gu_change}万千瓦时，占全时段电量的{percen_2024['谷']}%，较基准日的低谷时段占比增加{isImproveStr_Gu}{Gucompare_ratio}%。")
