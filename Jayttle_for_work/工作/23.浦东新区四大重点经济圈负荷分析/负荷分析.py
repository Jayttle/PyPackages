...
def run_main_0(read_file_path, save_file_path):
    ...
    df['日期'] = pd.to_datetime(df['日期'].astype(str), format='%Y_%m_%d', errors='coerce')  # 转换日期

    # 找到以'P'开头的列名
    time_columns = [col for col in df.columns if col.startswith('P')]
    time_columns = [col for col in df.columns if col.startswith('P')]
...
def run_main_4(fuhe_file_path, yongdian_file_path, save_file_path = False):
    """
    读取负荷的清单和用电的清单，用于匹配产业类别。
    """
    # 读取两个 Excel 文件
    fh_df = pd.read_excel(fuhe_file_path)
    yd_df = pd.read_excel(yongdian_file_path)
    ...
    # 使用 merge 将 '户号' 相同的行匹配，并将 '产业类别（第一、二、三产业、居民）' 列加到 fh_df 中
    merged_df = pd.merge(fh_df, yd_df[['户号', '产业类别（第一、二、三产业、居民）']], on='户号', how='left')

    # 检查 '产业类别（第一、二、三产业、居民）' 列是否存在空值，并打印出对应的户号
    missing_category = merged_df[merged_df['产业类别（第一、二、三产业、居民）'].isnull()]
    
    if not missing_category.empty:
        print("以下户号没有对应的产业类别:")
        print(missing_category['户号'].tolist())
    else:
        print("所有户号都成功匹配到产业类别。")
    ...
    # 计算按产业类别分类的平均负荷
    working_day_avg = working_day_data.groupby('产业类别（第一、二、三产业、居民）')[list(renamed_columns.values())].sum().reset_index()
    non_working_day_avg = non_working_day_data.groupby('产业类别（第一、二、三产业、居民）')[list(renamed_columns.values())].sum().reset_index()

    # 保存数据到新的工作表
    with pd.ExcelWriter(save_file_path, engine='openpyxl', mode='a') as writer:
        if '产业类别工作日负荷' in writer.book.sheetnames:
            del writer.book['产业类别工作日负荷']
        working_day_avg.to_excel(writer, sheet_name='产业类别工作日负荷', index=True)

        if '产业类别非工作日负荷' in writer.book.sheetnames:
            del writer.book['产业类别非工作日负荷']
        non_working_day_avg.to_excel(writer, sheet_name='产业类别非工作日负荷', index=True)

if __name__ == '__main__':
    print('------------------------run-----------------------')
    region_names = ["1.陆家嘴", "3.川沙", "4.东方枢纽"]
    region = "1.陆家嘴"

    read_file_path = rf"C:\Users\juntaox\Desktop\工作\23.浦东四大重点区域负荷分析\原始材料\浦东四大重点区域负荷数据-{region}.xlsx"
    ...
    # run_main_4(fuhe_file_path, yongdian_file_path, file_path)