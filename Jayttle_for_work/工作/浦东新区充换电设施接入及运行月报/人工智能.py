...
def evaluate_sarima(params, df_train, df_test):
    model = SARIMAX(df_train['人工智能用电量（千瓦时）'], 
                    order=params['order'], 
    ...
    df_test['误差'] = (df_test['预测'] - df_test['人工智能用电量（千瓦时）']) / df_test['人工智能用电量（千瓦时）']
    
    mse = mean_squared_error(df_test['人工智能用电量（千瓦时）'], df_test['预测'])
    return mse
...
def grid_search_sarima(df_train, df_test):
    ...
    # }
    param_grid = {
        'order': [(p, d, q) for p in [0] for d in [2] for q in [2]],
...
def train_and_predict_sarima(df_train, df_test):
    ...
    df_test['误差'] = (df_test['预测'] - df_test['人工智能用电量（千瓦时）']) / df_test['人工智能用电量（千瓦时）']
    comparison_df = df_test[['人工智能用电量（千瓦时）', '预测', '误差']]
    ...
    plt.plot(df_train.index, df_train['人工智能用电量（千瓦时）'], label='训练集')
    plt.plot(df_test.index, df_test['人工智能用电量（千瓦时）'], label='实际值', color='blue')
