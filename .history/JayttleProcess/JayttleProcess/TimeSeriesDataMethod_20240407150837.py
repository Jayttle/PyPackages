...
def calculate_cusum(time_series: List[TimeSeriesData], target: float) -> Tuple[List[float], List[float]]:
    """
    正向(CUSUM):正向CUSUM跟踪过程是否偏离了目标值。当过程偏离目标值时,正向CUSUM值将逐渐增加。
    正向CUSUM的主要目标是检测过程是否存在持续的上升或下降趋势,这可能表明过程不稳定。
    反向(CUSUM):反向CUSUM用于检测过程是否有持续的下降趋势或回归到目标值以下。
    当过程偏离目标值时,反向CUSUM值将逐渐减少。反向CUSUM的主要目标是检测过程是否有持续的下降趋势,
    这可能表示过程的某些参数发生了变化。
    """
