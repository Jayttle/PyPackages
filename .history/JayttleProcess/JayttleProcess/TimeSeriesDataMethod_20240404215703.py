...
def fit_Ridge_polynomial_trend(data: List[TimeSeriesData], degree: int=11, alpha: float=1.0):
    """使用岭回归拟合多项式趋势线"""
...
def fit_Lasso_polynomial_trend(data: List[TimeSeriesData], degree: int=11, alpha: float=1.0):
    """使用Lasso回归拟合多项式趋势线"""
    x = np.array(get_x_values(data))
    y = np.array([point.value for point in data])
...
def adaptive_polynomial_fitting(data: List[TimeSeriesData], initial_threshold: float) -> np.ndarray:
    ...
    while n < N - 1:
        r = 1
        while n + r < N - 1:
            segment_accelerations = accelerations[n:n+r+1]
        ...
        # 拟合多项式趋势线并计算相关系数
        best_degree = find_Ridge_best_degree_without_print(data)
...
plot_polynomial_trend(data[1:1000], complete_trend_line)
