    ...
    for start_time, rinex_list in rnx_files.items():
        ...
        for rnx_info in rinex_list:
            if rnx_info.format == FileFormat.CRX:
                process = subprocess.Popen([r"D:\Program Files (x86)\Software\OneDrive\C#\windows_C#\Cableway.Net7\Cableway.Download\bin\Release\net8.0-windows\Rinex\CRX2RNX.exe",
                                            rnx_info.file_info],
                                            stdout=subprocess.PIPE,
                                            stderr=subprocess.PIPE)
                process.wait()
...
directory_path = r"D:\Ropeway\GNSS\FTP\B011"

rnx_files: dict[datetime, list[RinexFileInfo]] = get_rnx_files_dict(directory_path)

crx_to_rnx(rnx_files)