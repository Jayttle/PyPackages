...
def evaluate_predictions(ridge_pred, lasso_pred, elastic_pred, polynomial_pred, data):
    ...
    # 计算模型参数数量
    num_parameters_ridge = len(ridge_model.coef_) + 1
    num_parameters_lasso = np.count_nonzero(lasso_model.coef_) + 1
    num_parameters_elastic = len(elastic_model.coef_) + 1
    num_parameters_polynomial = degree + 1  # 多项式拟合的参数数量为多项式次数 + 1

    # 计算 AIC 和 BIC
    aic_ridge = calculate_AIC(n, ridge_mse, num_parameters_ridge)
    bic_ridge = calculate_BIC(n, ridge_mse, num_parameters_ridge)
    aic_lasso = calculate_AIC(n, lasso_mse, num_parameters_lasso)
    bic_lasso = calculate_BIC(n, lasso_mse, num_parameters_lasso)
    aic_elastic = calculate_AIC(n, elastic_mse, num_parameters_elastic)
    bic_elastic = calculate_BIC(n, elastic_mse, num_parameters_elastic)
    aic_polynomial = calculate_AIC(n, polynomial_mse, num_parameters_polynomial)
    bic_polynomial = calculate_BIC(n, polynomial_mse, num_parameters_polynomial)
...
def visualize_predictions(data, ridge_pred, lasso_pred, elastic_pred, polynomial_pred):
    x = np.array(get_x_values(data))
...
polynomial_pred = fit_polynomial_trend(data, degree=13)  # 使用您之前定义的多项式拟合函数
# 岭回归拟合
ridge_pred = fit_Ridge_polynomial_trend(data, degree=10, alpha=1.0)

# Lasso回归拟合
lasso_pred = fit_Lasso_polynomial_trend(data, degree=10, alpha=1.0)

elastic_pred=fit_ElasticNet_polynomial_trend(data,degree=10, alpha=1.0)
# plot_polynomial_tread(data,elastic_pred)
#使用函数进行评估
evaluate_predictions(ridge_pred, lasso_pred,elastic_pred, data)
visualize_predictions(data, ridge_pred, lasso_pred, elastic_pred,polynomial_pred)
# 使用示例
#best_degree, best_r_squared = find_Ridge_best_degree(data)
