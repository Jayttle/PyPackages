...
def calculate_cusum_hotelling(time_series: List[TimeSeriesData], target: np.ndarray) -> List[float]:
    """
    计算CUSUM-Hotelling值

    Parameters:
        time_series (List[TimeSeriesData]): 时间序列数据
        target (np.ndarray): 目标值

    Returns:
        List[float]: CUSUM-Hotelling值
    """
    cusum_hotelling = [0]  # CUSUM-Hotelling值
    for data in time_series[1:]:  # 跳过第一个元素，因为已经初始化了
        # 计算Hotelling T2统计量
        values = np.array([entry.value for entry in time_series])
        values = values[:, np.newaxis]
        mean = np.mean(values, axis=0)
        covariance_matrix = np.cov(values, rowvar=False)
        t2 = np.matmul(np.matmul((values - mean), np.linalg.inv(covariance_matrix)), (values - mean).T)
        
        deviation = t2 - target
        cusum_hotelling.append(max(0, cusum_hotelling[-1] + deviation))
    return cusum_hotelling

def plot_cusum_hotelling(cusum_hotelling: List[float]) -> None:
    """
    绘制CUSUM-Hotelling控制图

    Parameters:
        cusum_hotelling (List[float]): CUSUM-Hotelling值
    """
    # 绘图
    plt.figure(figsize=(12, 6))
    plt.plot(cusum_hotelling, label='CUSUM-Hotelling', color='green')
    plt.axhline(y=0, color='k', linestyle='--')  # 目标线
    plt.xlabel('观测数目')
    plt.ylabel('CUSUM-Hotelling值')
    plt.title('CUSUM-Hotelling控制图')
    plt.legend()
    plt.show()
    plt.show()
# endregion
...
cusum_hotelling = calculate_cusum_hotelling(data, 0)
plot_cusum_hotelling(cusum_hotelling)
