...
def fit_Ridge_polynomial_trend(data, degree=5, alpha=1.0):
    """使用岭回归拟合多项式趋势线"""
    x = np.array([datetime.strptime(point.datetime, "%Y-%m-%d %H:%M:%S").timestamp() for point in data])
...
def plot_polynomial_trend( data,y_pred):
    """绘制多项式趋势线"""
...
y_pred=fit_Ridge_polynomial_trend(data)
plot_polynomial_trend(data,y_pred)
# 使用示例
