...
def crx_to_rnx(rnx_files: dict[datetime, list[RinexFileInfo]], delete_crx_file: bool = True, overwrite: bool = True) -> None:
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(convert_file, rnx_info, delete_crx_file): rnx_info for rinex_list in rnx_files.values() for rnx_info in rinex_list}
        for future in concurrent.futures.as_completed(futures):
            try:
                future.result()
            except Exception as e:
...
# merge_files_threadpool(to_process_rnx_file, merge_path)