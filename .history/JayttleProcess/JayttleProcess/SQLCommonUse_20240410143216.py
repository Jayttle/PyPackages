...
@log_function_call
def visualize_heatmap(aggregated_data: dict[datetime, list[str]], frequency: str) -> None:
    # 获取表名列表
    table_names = list(set(table for tables in aggregated_data.values() for table in tables))
    table_names.sort()
    ...
    if frequency == 'daily':
        ...
        plt.ylabel('Month starting')

    plt.xticks(np.arange(len(table_names)), table_names, rotation=45)
    plt.xlabel('Tables')
    plt.title('Data Presence Heatmap')
...
# 假设有如下聚合后的数据
aggregated_data = {
    datetime(2024, 4, 8): ['Table X', 'Table Y'],
    datetime(2024, 4, 9): ['Table Y'],
    datetime(2024, 4, 15): ['Table X', 'Table Z']
}

# 可视化数据（以每日为例）
visualize_heatmap(aggregated_data, frequency='daily')
