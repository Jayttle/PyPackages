...
def adaptive_polynomial_fitting(data: List[TimeSeriesData], initial_threshold: float) -> np.ndarray:
    ...
    while n < N - 1:
        ...
        #print(f"{count}: {num_trend_lines_added}\t{num_data_points_added}\tdegree: {best_degree}")
        count += 1
...
def calculate_control_limits(data):
    ...
    CL_X_bar = X_bar
    UCL_X_bar = X_bar + 2.66 * R  # 对于常见的样本大小，A2 = 2.66
    LCL_X_bar = X_bar - 2.66 * R

    R_values = []
    for i in range(len(data) - 1):
        R_values.append(abs(data[i].value - data[i+1].value))
    CL_R = np.mean(R_values)
    UCL_R = 3.267 * CL_R  # 对于常见的样本大小，D4 = 3.267
    LCL_R = 0
...
def plot_x_hart_control_chart(data):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(dates, values, 'b-', label='Values')
    plt.axhline(y=CL_X_bar, color='r', linestyle='--', label='CL')
    plt.axhline(y=UCL_X_bar, color='g', linestyle='--', label='UCL')
    plt.axhline(y=LCL_X_bar, color='g', linestyle='--', label='LCL')
    ...
    plt.plot(dates[:-1], R_values, 'b-', label='R Values')
    plt.axhline(y=CL_R, color='r', linestyle='--', label='CL')
...
cusum_values=cusum(data)
plot_cusum(data, cusum_values)
...
# complete_trend_line = adaptive_polynomial_fitting(data, 11)
# print(f"complete_trend_line: {len(complete_trend_line)}")
