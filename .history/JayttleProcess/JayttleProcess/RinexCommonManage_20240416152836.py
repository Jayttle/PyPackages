...
# 写入每天中持续时间为 01 小时的文件数量
for idx, (date, count) in enumerate(files_per_day_1h.items(), start=1):
    ws[f'A{idx}'] = date.strftime('%Y-%m-%d')
    ws[f'B{idx}'] = count

# 写入持续时间字符串的种类及对应的文件数量
for idx, (duration_str, count) in enumerate(duration_str_counts.items(), start=len(files_per_day_1h) + 3):
    ws[f'D{idx}'] = duration_str
    ws[f'E{idx}'] = count
