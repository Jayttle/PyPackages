...
def evaluate_predictions(ridge_pred, lasso_pred, elastic_pred, data):
    """评估 Ridge、Lasso 和 ElasticNet 的预测结果拟合效果"""
    ...
    # 比较 R方值
    max_r2 = max(ridge_r2, lasso_r2, elastic_r2)
    if max_r2 == ridge_r2:
        print("\nRidge回归模型的拟合效果最好")
    ...
    # 比较均方误差（MSE）
    min_mse = min(ridge_mse, lasso_mse, elastic_mse)
    if min_mse == ridge_mse:
        print("Ridge回归模型的拟合效果最好")
    elif min_mse == lasso_mse:
        print("Lasso回归模型的拟合效果最好")
    else:
        print("ElasticNet回归模型的拟合效果最好")
...
def visualize_predictions(data, ridge_pred, lasso_pred, elastic_pred):
    x = np.array(get_x_values(data))
    y_true = np.array([point.value for point in data])

    plt.figure(figsize=(10, 6))
    plt.scatter(x, y_true, color='blue', label='True Data', alpha=0.5)
    plt.plot(x, ridge_pred, color='red', label='Ridge Regression', linewidth=2)
    plt.plot(x, lasso_pred, color='green', label='Lasso Regression', linewidth=2)
    plt.plot(x, elastic_pred, color='orange', label='ElasticNet Regression', linewidth=2)
...
# 岭回归拟合
ridge_pred = fit_Ridge_polynomial_trend(data, degree=13, alpha=1.0)

# Lasso回归拟合
lasso_pred = fit_Lasso_polynomial_trend(data, degree=13, alpha=1.0)

elastic_pred=fit_ElasticNet_polynomial_trend(data)
plot_polynomial_tread(data,elastic_pred)
# 使用函数进行评估
# evaluate_predictions(ridge_pred, lasso_pred,elastic_pred, data)
visualize_predictions(data, ridge_pred, lasso_pred, elastic_pred)
# 使用示例
