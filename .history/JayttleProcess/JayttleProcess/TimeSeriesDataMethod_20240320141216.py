...
def ar_model_forecast(TimeSeriesData, lags=1, future_steps=10):
    ...
    # 计算均方根误差
    mse = mean_squared_error(X[-future_steps:], future_forecast)
...
def ma_model_forecast(TimeSeriesData, q, future_steps):
    # 提取时序数据的观测值
    X = np.array([data.value for data in TimeSeriesData])
    
    # 拟合MA模型
    model = ARIMA(X, order=(0, 0, q))  # 参数order=(0, 0, q)表示使用q阶移动平均模型
    model_fit = model.fit()
...
future_forecast, mse = ar_model_forecast(data)
print("未来预测的均方根误差：", mse)
# # 打印IMFs
# for i, imf in enumerate(imfs):
#     print(f"IMF {i+1}:")
