...
# 假设这是你从数据库中获取的更多数据名称和时间段
data = {
    'Data_A': [
        (datetime(2024, 4, 1), datetime(2024, 4, 3)),
        (datetime(2024, 4, 5), datetime(2024, 4, 7)),
        (datetime(2024, 4, 10), datetime(2024, 4, 13)),
    ],
    'Data_B': [
        (datetime(2024, 4, 2), datetime(2024, 4, 4)),
        (datetime(2024, 4, 8), datetime(2024, 4, 9)),
        (datetime(2024, 4, 11), datetime(2024, 4, 12)),
    ],
    'Data_C': [
        (datetime(2024, 4, 3), datetime(2024, 4, 5)),
        (datetime(2024, 4, 9), datetime(2024, 4, 10)),
        (datetime(2024, 4, 14), datetime(2024, 4, 15)),
    ],
    # 更多数据...
}
# 定义起始时间点
...
# 为每个数据名称创建并添加 1000 个具有秒级精度的 datetime 对象
for data_name in data.keys():
    # 使用随机增量来生成时间点，以此模拟现实世界的数据采集时间
    data[data_name] = [
        (start_datetime + timedelta(seconds=random.randint(1, 10)) * i,
         start_datetime + timedelta(seconds=random.randint(1, 10)) * i + timedelta(seconds=random.randint(1, 3600)))
        for i in range(1, 1001)
    ]

# 数据名称到数值的映射
data_name_mapping = {data_name: idx for idx, data_name in enumerate(data.keys(), start=1)}

plt.figure(figsize=(12, 8))

# 遍历每个数据名称及其对应的时间段，并在时间轴上标记出数据存在的时段
for data_name, time_ranges in data.items():
    idx = data_name_mapping[data_name]
    for start_time, end_time in time_ranges:
        plt.plot([start_time, end_time], [idx, idx], marker='|', markersize=10, label=data_name)
...
# 设置 y 轴标签为对应的数据名称
plt.yticks(list(data_name_mapping.values()), list(data_name_mapping.keys()))
