...
def plot_frequency_spectrum(transformed_values, sampling_rate):
    """绘制频谱的幅度谱图
    # 假设采样率为1（单位时间步长）
    sampling_rate = 1

    # 进行傅里叶变换并绘制幅度谱图
    transformed_values = fourier_transform(TimeSeriesData)
    plot_frequency_spectrum(transformed_values, sampling_rate)

    """
    N = len(transformed_values)  # 信号的长度
    frequencies = np.fft.fftfreq(N, 1/sampling_rate)  # 获取频率轴
    amplitudes = np.abs(transformed_values)  # 幅度谱

    # 排除零频率
    non_zero_indices = np.nonzero(frequencies)
    non_zero_frequencies = frequencies[non_zero_indices]
    non_zero_amplitudes = amplitudes[non_zero_indices]

    plt.plot(non_zero_frequencies, non_zero_amplitudes)
    plt.xlabel('频率')
    plt.ylabel('幅度')
    plt.title('频谱的幅度谱图')
    plt.show()
