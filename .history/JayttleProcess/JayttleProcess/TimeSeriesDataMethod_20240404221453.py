...
def adaptive_polynomial_fitting(data: List[TimeSeriesData], initial_threshold: float) -> np.ndarray:
    ...
    while n < N - 1:
        ...
        # 将当前窗口的拟合趋势线添加到列表中，并记录添加次数和数据量
        num_trend_lines_added = len(trend_lines) + 1
        num_data_points_added = len(segment_data)
        print(f"Iteration {count}:Added trend line {num_trend_lines_added} with {num_data_points_added} data points.")
        count += 1
