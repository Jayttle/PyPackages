...
def aggregate_data_by_time(time_data: list[datetime], frequency: str) -> Dict[dt.datetime, List[str]]:
    aggregated_data = {}
    ...
    for time_point in time_data:
        ...
        if frequency == 'daily':
            ...
            # 将时间调整为所在周的周一的午夜
            aggregated_time = time_point - dt.timedelta(days=time_point.weekday())
...
# 调用数据聚合函数（以每日为例）
aggregated_data_daily = aggregate_data_by_time(input_data, frequency='daily')
print("Aggregated data (daily):", aggregated_data_daily)

# 调用数据聚合函数（以每周为例）
aggregated_data_weekly = aggregate_data_by_time(input_data, frequency='weekly')
print("Aggregated data (weekly):", aggregated_data_weekly)

# 调用数据聚合函数（以每月为例）
aggregated_data_monthly = aggregate_data_by_time(input_data, frequency='monthly')
print("Aggregated data (monthly):", aggregated_data_monthly)
