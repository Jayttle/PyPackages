...
def export_missing_percentage_to_file(daily_missing_percentage: dict[datetime, float]) -> None:
    ...
    """
    # 将字典转换为 DataFrame
    df = pd.DataFrame(list(daily_missing_percentage.items()), columns=['Date', 'Missing Percentage'])
...
def load_missing_intervals(file_path: str) -> list[tuple[datetime.datetime, datetime.datetime, int]]:
    """
    ...
    返回：
        List[Tuple[datetime.datetime, datetime.datetime, int]]: 包含缺失时间段的列表
    """
    missing_intervals = []
...
# 假设 daily_missing_percentage 是一个字典，键为日期，值为每天的缺失百分比
# 例如：daily_missing_percentage = {'2024-01-01': 10.5, '2024-01-02': 5.3, ...}
