...
def plot_polynomial_trend(data: List[TimeSeriesData], y_pred: List[float], SaveFilePath: Optional[str]=None):
    """绘制多项式趋势线"""
    # 提取日期时间和数值信息
...
def find_Ridge_best_degree_without_print(data:List[TimeSeriesData]) -> int:
    ...
    for degree in range(1, 11):  # 假设循环从1到21的范围
        # 创建 Polynomial 特征转换器
        polynomial_features = PolynomialFeatures(degree=degree)
        X_poly = polynomial_features.fit_transform(x.reshape(-1, 1))
...
def calculate_hotelling_t2(data: List[TimeSeriesData]) -> np.ndarray:
    # 提取变量的观测值
    values: np.ndarray = np.array([entry.value for entry in data])
    
    # 计算变量的平均值
    mean: np.ndarray = np.mean(values, axis=0)
    
    # 计算变量的协方差矩阵
    covariance_matrix: np.ndarray = np.cov(values, rowvar=False)
    
    # 计算Hotelling T2统计量
    t2: np.ndarray = np.matmul(np.matmul((values - mean), np.linalg.inv(covariance_matrix)), (values - mean).T)
    
    return t2

def cusum_hotelling(data: List[TimeSeriesData], threshold: float=1) -> List[float]:
    cusum_values: List[float] = [0]  # 初始CUSUM值为0
    
    for i in range(1, len(data)):
        t2: np.ndarray = calculate_hotelling_t2(data[:i+1])  # 计算当前时间点的Hotelling T2统计量
        cusum_values.append(max(0, cusum_values[i-1] + t2 - threshold))  # 计算CUSUM值
    
    return cusum_values

def plot_cusum_hotelling(data: List[TimeSeriesData], cusum_values: List[float],threshold: float, SaveFilePath: Optional[str]=None) -> None:
    # 绘制控制图
    plt.plot(range(len(data)), cusum_values, label='CUSUM-Hotelling')
    plt.axhline(y=threshold, color='r', linestyle='--', label='Threshold')
    plt.xlabel('Time')
    plt.ylabel('CUSUM Value')
    plt.title('CUSUM-Hotelling Control Chart')
    plt.legend()
    if SaveFilePath is not None:
        plt.savefig(SaveFilePath) 
    else:
...
# 计算CUSUM-Hotelling控制图
threshold: float = 100  # 设定阈值
cusum_values: List[float] = cusum_hotelling(data, threshold)
plot_cusum_hotelling(data, cusum_values, threshold)
