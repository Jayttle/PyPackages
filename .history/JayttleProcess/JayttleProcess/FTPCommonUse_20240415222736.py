...
def load_ftp_files():
    ...
    try:
        ...
        if 'towerbase2' in file_list:
            ftp.cwd('towerbase2')  # 切换到 tower5 文件夹
            tower3_files = ftp.nlst()  # 获取 tower5 文件夹下的文件列表

            # 输出 tower5 文件夹下的文件数量
            print(f"towerbase2 文件夹中有 {len(tower3_files)} 个文件。")

            # 将 tower3 文件夹下的文件名逐行保存到本地的 txt 文件中
            txt_file_path = os.path.join(local_save_path, 'towerbase2_files.txt')
            with open(txt_file_path, 'w') as txt_file:
                for file_name in tower3_files:
                    txt_file.write(file_name + '\n')

            print(f"towerbase2 文件夹中的文件名已保存到本地: {txt_file_path}")
