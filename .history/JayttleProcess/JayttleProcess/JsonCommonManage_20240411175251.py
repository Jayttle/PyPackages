...
class Product:
    ...
    def years_of_usage(self) -> float:
        today = datetime.now()
        years = (today - self.time).days / 365.25
...
class ProductList:
    ...
    def calculate_usage_and_price(self) -> None:
        """
        计算每个设备的使用时间和日均价格，并打印结果。
        """
        for product in self.products:
            years_usage = product.years_of_usage()
            daily_price = product.daily_price()
            print(f"{product.category}-{product.model}: 使用时间：{years_usage} 年，日均价格：{daily_price} 元")
