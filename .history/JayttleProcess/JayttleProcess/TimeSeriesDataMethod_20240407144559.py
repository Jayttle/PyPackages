...
def plot_acceleration(data: List[TimeSeriesData], accelerations: List[float]) -> None:
...
def plot_cusum_hotelling(data: List[TimeSeriesData], cusum_values: List[float],threshold: float) -> None:
    # 绘制控制图
...
def plot_x_hart_control_chart(data):
...
def plot_control_limits_ewma(data: List[TimeSeriesData], alpha=0.2) -> None:
    # 将数据转换为列表以便计算
...
# endregion
# region 概率论相关
def kernel_density_estimation(data: List[TimeSeriesData]) -> None:
