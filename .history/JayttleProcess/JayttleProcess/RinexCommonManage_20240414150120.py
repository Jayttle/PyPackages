...
def filter_and_group_folders(directory: str) -> dict[int, dict[int, list[int]]]:
    ...
    for key, value in folder_groups.items():
        ...
        # 处理个数为4的情况
        for group_key, group_value in grouped_folders.items():
            if len(group_value) == 4:
                directory_path = os.path.join(directory, f"{key:08d}{group_key:02d}")
                rnx_files = get_rnx_files(directory_path)
                group_value.extend(rnx_files)  # 将RinexFileInfo对象列表添加到group_value中
