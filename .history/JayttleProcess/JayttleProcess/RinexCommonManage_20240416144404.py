...
# 创建一个字典来存储每天01小时内的文件数量
files_per_day_1h = defaultdict(int)

# 遍历 RinexFileInfo 列表
for file_info in rinex_files_info:
    # 获取文件的起始时间和持续时间
    start_time = file_info.start_gps_time
    duration_str = file_info.duration_str

    # 将持续时间字符串转换为持续时间（假设持续时间格式为"H"）
    duration_hours = int(duration_str[:-1])  # 去除最后的 "H" 并转换为整数

    # 计算文件的结束时间
    end_time = start_time + timedelta(hours=duration_hours)

    # 检查文件是否在01小时内
    if start_time.hour == 1 and end_time.hour == 2:
        # 如果是，则增加对应日期的文件数量
        files_per_day_1h[start_time.date()] += 1

# 输出每天01小时内的文件数量
for date, count in files_per_day_1h.items():
    print(f"On {date.strftime('%Y-%m-%d')}, {count} files within 01H.")