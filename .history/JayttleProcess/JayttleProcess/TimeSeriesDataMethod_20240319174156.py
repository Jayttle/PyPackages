...
def bayesian_change_point_detection(data, prior_mean, prior_stddev, hazard):
    """
    使用贝叶斯变点检测方法检测时间序列数据中的突变点。

    :param data: TimeSeriesData 类型的列表，包含 value 和 datetime 属性。
    :param prior_mean: 先验分布的均值。
    :param prior_stddev: 先验分布的标准差。
    :param hazard: 突变点的先验概率。
    :return: 突变点的位置列表。
    """
    # 提取 value 值
    ...
    n = len(values)
    posterior_probs = np.zeros(n)

    # 初始化先验分布
    prior = np.ones(n+1) * 1.0 / n
    prior[0] = hazard

    for t in range(n):
        # 根据当前观测值计算似然函数
        likelihood = np.exp(-0.5 * (values[t] - values) ** 2)
        ...
        # 更新先验概率
        prior = posterior * (1 - hazard)
        prior[1:] += prior[:-1] * hazard

        # 计算变点位置的后验概率
        posterior_probs[t] = np.sum(posterior[:-1])

    # 找到后验概率的峰值作为变点位置
    change_points = np.where(posterior_probs == np.max(posterior_probs))[0]
...
prior_stddev = 1  # 先验分布的标准差
hazard = 0.01  # 突变点的先验概率

change_points = bayesian_change_point_detection(data, prior_mean, prior_stddev, hazard)
print("突变点位置:", change_points)
# 可视化结果
fig, ax = plt.subplots()
ax.plot(range(len(data)), [entry.value for entry in data], linestyle='-', color='blue', label='Data')
ax.vlines(change_points, np.min(values), np.max(values), linestyle='--', color='red', label='Change Points')
ax.legend()
plt.show()