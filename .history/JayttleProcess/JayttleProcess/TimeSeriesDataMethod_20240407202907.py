...
def elbow_method(complete_trend_line: np.ndarray, max_clusters: int):
    ...
    # 计算每个簇中的成员数
    cluster_counts = [np.sum(kmeans.labels_ == i) for i in range(1, max_clusters + 1)]

    # 绘制每个簇序号的成员数
    plt.bar(range(1, max_clusters + 1), cluster_counts)
    plt.xlabel('簇序号')
    plt.ylabel('成员数')
    plt.title('每个簇序号的成员数') 
    plt.show()
...
num_clusters = 3
cluster_labels = cluster_analysis_DBSCAN(complete_trend_line)
