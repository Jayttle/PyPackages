...
def plot_rolling_std(data: List[TimeSeriesData], window_size: int, multiplier: float = 2):
    """绘制滚动标准差的控制图"""
    std_values = calculate_rolling_std(data, window_size)
    upper_limit = [multiplier * std for std in std_values]
    lower_limit = [-multiplier * std for std in std_values]

    plt.figure(figsize=(12, 6))
    plt.plot(std_values, label='Rolling Std')
    plt.plot(upper_limit, label='Upper Limit', linestyle='--', color='red')
    plt.plot(lower_limit, label='Lower Limit', linestyle='--', color='red')
    plt.xlabel('Data Point')
    plt.ylabel('Rolling Std')
    plt.title('Rolling Standard Deviation Control Chart')
    plt.legend()
    plt.grid(True)
    plt.show()
