...
# 将数据按缺失百分比降序排列
sorted_data = sorted(daily_missing_percentage.items(), key=lambda x: x[1], reverse=True)
dates = [date for date, _ in sorted_data]
percentages = [percentage for _, percentage in sorted_data]

# 创建华夫饼图
plt.figure(figsize=(10, 6))
plt.barh(dates, percentages, color='skyblue')
plt.xlabel('Missing Percentage')
plt.ylabel('Date')
plt.title('Daily Missing Percentage')
plt.gca().invert_yaxis()  # 反转y轴，让日期从上到下显示
plt.tight_layout()
plt.show()