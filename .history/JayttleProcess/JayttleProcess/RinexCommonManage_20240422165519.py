...
def proj_1(): 
    ...
    rinex_files_info: list[RinexFileInfo] = read_rinex_files_info_version2(file_list_path)
    
    # 指定时间范围
    start_hour= 16
    end_hour = 19
    # 统计每天指定时间范围内文件的数量，并按日期和标记站名返回文件数的字典。
    files_in_hour_range: dict[datetime.date, dict[str, int]] = count_files_in_hour_range(rinex_files_info, start_hour, end_hour)
