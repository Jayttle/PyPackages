...
# 将数据转换为适合华夫饼图的格式
data = {str(date): percentage for date, percentage in daily_missing_percentage.items()}

# 创建华夫饼图
fig = plt.figure(
    FigureClass=Waffle,
    rows=10,  # 定义华夫饼图的行数
    columns=18,  # 定义华夫饼图的列数
    values=data,  # 数据
    labels=[f"{date}\n{percentage}%" for date, percentage in daily_missing_percentage.items()],  # 添加数值百分比标签
