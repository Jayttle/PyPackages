...
def moving_average(time_series_data, window_size):
    """计算移动平均值"""
    values = time_series_data.value
    datetimes = time_series_data.datetime
    n = len(values)
    moving_avg = []
...
def plot_moving_average(time_series_data, window_size):
    """绘制移动平均线"""
    avg_values = moving_average(time_series_data, window_size)
    datetimes = time_series_data.datetime[window_size - 1 :]

    plt.plot(time_series_data.datetime, time_series_data.value, label="原始数据")
    plt.plot(datetimes, avg_values, label="移动平均")
    plt.xlabel('日期', fontproperties=font)  # 指定中文标签
    plt.ylabel('数值', fontproperties=font)  # 指定中文标签
    plt.title('移动平均线', fontproperties=font)  # 指定中文标签
    plt.legend(prop=font)  # 指定中文标签
    plt.show()
