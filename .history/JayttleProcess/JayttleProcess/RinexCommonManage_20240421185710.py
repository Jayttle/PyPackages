...
def merge_files_by_hour_multithread_version2(groups: list[list[RinexFileInfo]], merge_path: str) -> None:
    """
    使用多线程将按小时分组的 RinexFileInfo 对象列表合并到指定路径。
    ...
    参数：
        ...
        merge_path (str): 合并后的文件存储路径。
    """
    threads = [threading.Thread(target=MergeFiles.merge_files, args=(file_group, merge_path)) for file_group in groups]
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
...
merge_files_by_hour_multithread_version2(to_process_rnx_file, merge_path)