...
# 计算每个数据存在时段的长度
data_lengths = {key: sum([(end - start).days + 1 for start, end in value]) for key, value in data.items()}

# 创建柱状图
plt.figure(figsize=(12, 8))

# 绘制柱子
plt.bar(data_lengths.keys(), data_lengths.values())

# 设置图形标题和轴标签
plt.title('Data Presence Over Time')
plt.xlabel('Data Name')
plt.ylabel('Time Period Length (Days)')
