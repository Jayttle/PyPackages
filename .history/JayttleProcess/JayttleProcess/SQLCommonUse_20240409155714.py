...
time_ranges = sum(data.values(), [])

# 获取时间段的最大最小值
min_time = min(start for start, end in time_ranges)
max_time = max(end for start, end in time_ranges)
...
for idx, data_name in enumerate(data_names):
    ...
    for start, end in time_ranges:
        data_heights.append((start <= min_time + duration * idx / len(data_names)) and (end >= min_time + duration * (idx + 1) / len(data_names)))
    # 绘制圆柱
    ax.barh(idx, sum(data_heights), align='center', height=0.5, color=np.random.rand(3,), label=data_name)
