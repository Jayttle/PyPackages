...
class RinexFileInfo:
    def __init__(self, input_path: str, ftp_file: bool):
        ...
        }.get(self.station_name[0], StationType.Unknown)
        ...
        self.file_type = {  # 文件类型，根据文件名中的第二个字符确定，如果未知则为 Unknown
            'O': RinexFileType.O,
            'N': RinexFileType.N
        }.get(file_type, RinexFileType.Unknown)
        ...
        self.format = {     # 文件格式，根据文件扩展名确定，如果未知则为 Unknown
            "crx": FileFormat.CRX,
            "rnx": FileFormat.RNX
        }.get(split1[1].lower(), FileFormat.Unknown)
...
# 创建 RinexFileInfo 实例
file_info = RinexFileInfo(r"D:\Ropeway\GNSS\B011\B01100000_R_20233491200_01H_01S_MO.rnx", ftp_file=True)
