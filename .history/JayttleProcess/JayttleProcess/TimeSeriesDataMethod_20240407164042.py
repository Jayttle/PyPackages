...
def calculate_cumulative_std_with_break(data: List[TimeSeriesData], threshold: float = 1.5) -> List[float]:
    """计算每个点到data[0:n]的标准差，如果超过阈值则隔断"""
    values = [item.value for item in data]  # 提取时序数据中的值
    std_values = []
...
def plot_rolling_std_with_limits(data: List[TimeSeriesData], window_size: int, multiplier: float = 2):
    """绘制滚动标准差的控制图，并使用中心线+标准差形式定义上下限"""
    std_values = calculate_rolling_std(data, window_size)
    center_line = np.mean(std_values)
    upper_limit = center_line + multiplier * np.std(std_values)
    lower_limit = center_line - multiplier * np.std(std_values)

    plt.figure(figsize=(12, 6))
    plt.plot(std_values, label='Rolling Std')
    ...
    plt.title('Rolling Standard Deviation Control Chart with Limits')
    plt.legend()
    plt.grid(True)
    plt.show()
...
plot_rolling_std_with_limits(data, 60 ,2)
...
std_values = calculate_cumulative_std_with_break(data, threshold)
...
complete_trend_line = adaptive_polynomial_fitting_in_std(data)
# print(f"Origin: {len(data)}, complete_trend_line: {len(complete_trend_line)}")

show_prcoess_time(start_time, "adaptive_polynomial_fitting")
plot_polynomial_trend(data[1: len(data) - 1], complete_trend_line)
