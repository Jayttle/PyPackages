...
def create_marker_name_excel(marker_name_files_per_day_1h: dict[datetime.date, dict], file_path: str = "marker_name_statistics.xlsx") -> None:
    ...
    # 用于跟踪已写入的 marker_name
    written_marker_names = set()

    for marker_name, files_per_day in marker_name_files_per_day_1h.items():
        # 如果 marker_name 还没有写入，则将其写入到第一列
        if marker_name not in written_marker_names:
            column_idx = 2  # 第一次写入到第二列
            written_marker_names.add(marker_name)
        else:
            column_idx = 3  # 第二次写入到第三列

        for idx, (date, count) in enumerate(files_per_day.items(), start=2):
            ws_1h[f'A{idx}'] = date.strftime('%Y-%m-%d')
            ws_1h[f'B{idx}'] = marker_name
            ws_1h.cell(row=idx, column=column_idx).value = count
