...
def visualize_missing_data(start_time: datetime, end_time: datetime, missing_intervals):
    # Calculate the total duration in weeks
    total_duration = (end_time - start_time).days
    total_weeks = total_duration // 7 + 1

    # Initialize lists to store the percentage of missing time for each week
    missing_percentage = [0] * total_weeks

    # Calculate the percentage of missing time for each week
    for start, end, duration in missing_intervals:
        week_start = (start - start_time).days // 7
        week_end = (end - start_time).days // 7
        for i in range(week_start, week_end + 1):
            missing_percentage[i] += duration / (7 * 24 * 3600)

    # Create a grid plot
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.set_xticks(range(total_weeks + 1))
    ax.set_yticks(range(int(max(missing_percentage)) + 2))  # Set y-axis range based on the maximum missing percentage
    ax.set_xticklabels([f'Week {i+1}' for i in range(total_weeks)] + [''], rotation=45)
    ax.set_xlabel('Week')
    ax.set_ylabel('Missing Time (hours)')
    ax.set_title('Grid Plot for Missing Data')

    # Plot the bar chart
    for i, percentage in enumerate(missing_percentage):
        ax.bar(i, percentage, width=0.8, align='center', alpha=0.8)
...
# 缺失数据时间段
missing_intervals = [
    (datetime(2023, 4, 13, 18, 31, 50, 874000), datetime(2023, 4, 18, 0, 42, 41, 391000), 367850),
    (datetime(2023, 4, 18, 12, 1, 21, 406000), datetime(2023, 4, 18, 12, 3, 31, 984000), 130),
    (datetime(2023, 4, 18, 17, 41, 11, 329000), datetime(2023, 4, 18, 23, 23, 11, 454000), 20520),
    (datetime(2023, 4, 19, 5, 0, 51, 532000), datetime(2023, 4, 19, 10, 42, 51, 423000), 20519),
    (datetime(2023, 4, 19, 16, 22, 21, 405000), datetime(2023, 4, 19, 22, 2, 53, 781000), 20432)
]
...
# 可视化缺失数据的网格图
visualize_missing_data(start_time, end_time, missing_intervals)