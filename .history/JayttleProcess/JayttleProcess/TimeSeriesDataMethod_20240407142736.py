...
def adaptive_polynomial_fitting_in_cusum(data: List[TimeSeriesData]) -> np.ndarray:
    ...
    while n < N - 1:
        ...
        while n + r < N - 1:
            segment_data = data[n:n+r]
            cusum_values = []
            cusum_values = cusum(segment_data)
            print(f"{cusum_values[len(cusum_values)-1]}\t{initial_threshold}")
            if cusum_values[len(cusum_values)-1] > initial_threshold:
                break
            r += 1
        # 拟合多项式趋势线并计算相关系数
        best_degree = find_Ridge_best_degree_without_print(segment_data)
        trend_line = fit_Ridge_polynomial_trend(segment_data, best_degree)
        correlation = np.corrcoef(np.array([point.value for point in segment_data]), trend_line)[0, 1]
        # print(f"tl: {len(trend_line)}, sgd:{len(segment_data)} n:{n} r:{r}")
...
def detect_cusum_diff_threshold(cusum_values: List[float], threshold: float) -> List[float]:
    """
    ...
    return change_points
def detect_cusum_window(cusum_values: List[float], threshold: float, window_size: int):
    """
    滑动窗口来检测连续的CUSUM值
...
def calculate_rolling_std(data: List[TimeSeriesData], window_size: int):
    """滚动标准差来检测波动性的变动"""
