...
# 打开文件并读取内容
with open('output_table.txt', 'r', encoding='utf-8') as file:
    # 初始化一个空列表来存储数据行
    data_rows = []

    # 逐行读取文件内容
    for line in file:
        # 如果当前行不以 '|' 开头或结尾，则将其添加到数据行列表中
        if not line.startswith('|') and not line.endswith('|'):
            # 将每行按 '|' 分割为单元格数据
            cells = line.strip().split('|')
            # 去除单元格中的空格并将数据添加到数据行列表中
            data_rows.append([cell.strip() for cell in cells if cell.strip()])
