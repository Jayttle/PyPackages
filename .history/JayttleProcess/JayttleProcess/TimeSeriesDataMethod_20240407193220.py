...
def visualize_clusters(complete_trend_line: np.ndarray, cluster_labels: np.ndarray):
    """可视化聚类分析结果"""
    plt.figure(figsize=(10, 6))

    # 绘制散点图，按照聚类标签进行着色
    plt.scatter(np.arange(len(complete_trend_line)), complete_trend_line, c=cluster_labels, cmap='viridis', alpha=0.5)
    ...
    for i in range(max(cluster_labels) + 1):
        ...
        plt.scatter(np.arange(len(complete_trend_line))[cluster_labels == i], complete_trend_line[cluster_labels == i], label=f'Cluster {i}', alpha=0.5)
...
def hierarchical_clustering(data: np.ndarray, method='average', metric='euclidean') -> np.ndarray:
    """层次聚类"""
    linkage_matrix = hierarchy.linkage(data, method=method, metric=metric)
    cluster_labels = hierarchy.fcluster(linkage_matrix, t=2, criterion='maxclust')  # 这里设置阈值 t=2 作为示例

    return cluster_labels

# 可视化聚类结果
def plot_dendrogram(data, method='average', metric='euclidean'):
    linkage_matrix = hierarchy.linkage(data, method=method, metric=metric)
    plt.figure(figsize=(10, 6))
...
# 使用示例
scaled_complete_trend_line = normalize_features(complete_trend_line.reshape(-1, 1))
plot_polynomial_trend(data, scaled_complete_trend_line)
# cluster_labels = hierarchical_clustering(complete_trend_line)
# visualize_clusters(complete_trend_line, cluster_labels)
plot_dendrogram(complete_trend_line)
