...
from sklearn.metrics import root_mean_squared_error, mean_absolute_error, r2_score, mean_squared_error, silhouette_score, davies_bouldin_score, calinski_harabasz_score
...
def cluster_analysis_with_guss(complete_trend_line: np.ndarray, num_clusters: int) -> np.ndarray:
...
def cluster_analysis_with_score(complete_trend_line: np.ndarray, num_clusters: int) -> tuple:
    """使用 K-means 聚类算法对趋势线进行聚类分析"""
    # 将趋势线数组重塑为列向量
    trend_line_vector = complete_trend_line.reshape(-1, 1)
...
# 执行聚类分析
num_clusters = 3
cluster_labels, silhouette, davies_bouldin, calinski_harabasz = cluster_analysis_with_score(complete_trend_line, num_clusters)
