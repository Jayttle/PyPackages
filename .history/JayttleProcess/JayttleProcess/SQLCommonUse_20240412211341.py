...
for date in (min_date + timedelta(days=i) for i in range((max_date - min_date).days + 1)):
    combined_dict[date] = {
        'daily_availability_met': daily_availability_met.get(date, 0),
        'daily_availability_accelerometer': daily_availability_accelerometer.get(date, 0),
        'daily_availability_tiltmeter': daily_availability_tiltmeter.get(date, 0)
    }
