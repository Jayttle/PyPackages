...
def parse_tabulated_file(filepath: str):
    with open(filepath, 'r', encoding='utf-8') as file:
        lines = file.readlines()

    # 假设表格的边界样式是 '+'，通过找到包含 '+' 的行确定表格的头部和尾部
    start = [i for i, line in enumerate(lines) if '+' in line][1] + 1
    end = [i for i, line in enumerate(lines) if '+' in line][-1]
    headers = lines[start].strip().split('|')[1:-1]
...
headers, data = parse_tabulated_file('output_table.txt')
print(headers)
print(data)