...
def perform_ljung_box_test(residuals, lags=10):
    """执行Ljung-Box检验"""
    lb_test_statistic, p_value = acorr_ljungbox(residuals, lags=lags)
    return lb_test_statistic, p_value

def plot_residuals(residuals, SaveFilePath=None):
    """绘制残差图"""
...
def find_Ridge_best_degree(data):
    ...
    for degree in range(1, 21):  # 假设循环从1到21的范围
        ...
        print(f"Degree {degree}: R方值 = {r_squared}, 矫正R方值 = {adjusted_r_squared}, SSE = {sse}, MAE = {mae}, MAPE = {mape}, AIC = {aic}")
    ...
    print(f"最佳Degree: {best_degree}, 对应的最佳R方值: {best_r_squared}, 最佳MAE: {best_mae}, 最佳MAPE: {best_mape}")
