...
class RinexFileInfo:
    def __init__(self, input_path: str, ftp_file: bool):
        ...
        self.start_gps_time: str = self.get_time_from_string(self.start_gps_time_str)  # 起始GPS时间，使用自定义函数从字符串中获取
        self.duration_str: str = split[3]  # 持续时间字符串，从文件名中获取
        self.duration: str = self.get_duration_from_string(self.duration_str)  # 持续时间，使用自定义函数从字符串中获取
...
def create_marker_name_excel(marker_name_files_per_day_1h: dict[str, dict], file_path: str = "marker_name_statistics.xlsx") -> None:
    # 创建一个新的工作簿
    wb = Workbook()

    # 写入每个 marker_name 的每天中持续时间为 01 小时的文件数量，包括标题行
    ws_marker_name = wb.active
    ws_marker_name.title = "Marker Name Statistics"
    ws_marker_name['A1'] = "Date"

    # 写入 marker_name 标题行
    col_idx = 2
    for marker_name in marker_name_files_per_day_1h.keys():
        ws_marker_name.cell(row=1, column=col_idx, value=marker_name)
        col_idx += 1

    # 写入数据
    for idx, (date, files_per_day) in enumerate(marker_name_files_per_day_1h.items(), start=2):
        ws_marker_name.cell(row=idx, column=1, value=date.strftime('%Y-%m-%d'))
        for col_idx, count in enumerate(files_per_day.values(), start=2):
            ws_marker_name.cell(row=idx, column=col_idx, value=count)

    # 设置标题行和数据单元格的对齐方式
    for row in ws_marker_name.iter_rows(min_row=1, max_row=1):
        for cell in row:
            cell.alignment = Alignment(horizontal='center', vertical='center')
    for row in ws_marker_name.iter_rows(min_row=2, max_row=len(marker_name_files_per_day_1h)+1):
        for cell in row:
            cell.alignment = Alignment(horizontal='center')

    # 设置列宽
    for col in ws_marker_name.columns:
        max_length = 0
        ...
        ws_marker_name.column_dimensions[column].width = adjusted_width
...
# 输出到 Excel 表格中
create_marker_name_excel(marker_name_files_per_day_1h)
