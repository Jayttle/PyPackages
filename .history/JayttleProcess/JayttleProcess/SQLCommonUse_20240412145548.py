...
@log_function_call
def calculate_daily_missing_percentage(
    start_time: datetime, 
    end_time: datetime, 
    missing_intervals: list[tuple[datetime, datetime, int]]
) -> list[float]:
    ...
    返回：
    - list[float]，每天的缺失百分比列表
    """

    # 初始化列表以存储每天的缺失百分比
    daily_missing_percentage = []

    # 遍历从 start_time 到 end_time 的每一天
    ...
    while current_day <= end_time:
        ...
        daily_missing_percentage.append(missing_percentage)
...
with open(file_path, "r") as file:
    for line in file:
        ...
        missing_intervals.append((min_time, max_time, missing_seconds))
...
daily_missing_percentage = calculate_daily_missing_percentage(start_time, end_time, missing_intervals)
for item in daily_missing_percentage:
    print(item)