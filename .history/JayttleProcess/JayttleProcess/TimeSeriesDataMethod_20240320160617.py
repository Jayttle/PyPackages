...
def visualize_harmonic_frequency_pie_chart(peak_frequencies, peak_values):
    """
    绘制谐波频率的饼图
    输入参数:
    - peak_frequencies: 谐波频率
    ...
    plt.pie(peak_values, labels=peak_frequencies, autopct='%1.1f%%')
    plt.title('谐波频率分布')
    plt.show()
...
def evaluate_forecast(TimeSeriesData, predicted_values):
    ...
    actual_values = [point.value for point in data[-len(predicted_values):]]
...
transformed_values=fourier_transform(data)
# 提取频域特征
sample_rate = 1  # 假设采样率为每秒钟1个样本
frequency_features = extract_frequency_features(transformed_values, sample_rate)

# 打印频域特征示例
visualize_frequency_features(frequency_features)

# 分析频率成分并查找峰值频率
threshold = None  # 可选参数,设定峰值检测的阈值,默认为None
peak_frequencies, peak_values = analyze_frequency_components(frequency_features, threshold)

visualize_harmonic_frequency_distribution(peak_frequencies)
# 假设你有一个谐波频率列表 peak_frequencies 和对应的幅度值列表 peak_values
visualize_harmonic_frequency_bar_chart(peak_frequencies, peak_values)
visualize_harmonic_frequency_pie_chart(peak_frequencies, peak_values)