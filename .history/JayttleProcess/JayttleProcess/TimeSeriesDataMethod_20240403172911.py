...
def adaptive_polynomial_fitting(accelerations, initial_threshold, degree):
    threshold = initial_threshold
    n = 0
    while n < len(accelerations):
        r = 1
        while n + r < len(accelerations):
            # 计算加速度并检查阈值
            accelerations = calculate_acceleration(accelerations[n:n+r+1])
            if all(a < threshold for a in accelerations):
                break
            r += 1
        
        # 拟合多项式趋势线并计算相关系数
        trend_line = fit_polynomial_trend_line(accelerations, r, degree)
        correlation = np.corrcoef([accelerations[i].value for i in range(n, n+r)], trend_line)[0, 1]
