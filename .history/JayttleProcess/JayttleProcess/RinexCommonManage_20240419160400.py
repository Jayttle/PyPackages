...
def count_files_in_hour_range(rinex_files_info: list[RinexFileInfo], start_hour: int, end_hour: int) -> dict[datetime.date, int]:
    """
    统计每天指定时间范围内文件的数量。
    ...
    返回：
        dict[datetime.date, int]: 每天指定时间范围内文件的数量。
    """
    files_in_hour_range = defaultdict(int)
    ...
    for rinex_file in rinex_files_info:
        ...
        if start_hour <= file_hour <= end_hour:
            # 在字典中增加或更新日期对应的文件数
            files_in_hour_range[file_date] += 1
    
    return files_in_hour_range
...
# 统计每天文件数并排序
sorted_files_per_day = count_files_per_day(rinex_files_info)
