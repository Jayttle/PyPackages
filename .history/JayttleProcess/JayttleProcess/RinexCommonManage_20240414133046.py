...
def filter_and_group_folders(directory: str) -> list[list[int]]:
    ...
    for folder_name in os.listdir(directory):
        if folder_name[-2:].isdigit():
            ...
            # 满足条件才进行分组
            if (12 <= last_two_digits <= 15) or (16 <= last_two_digits <= 19) or (20 <= last_two_digits <= 23):
                folder_groups[first_eight_digits].append(last_two_digits)

    # 筛选满足要求的分组
    valid_groups = [group for group in folder_groups.values() if len(group) == 4]
