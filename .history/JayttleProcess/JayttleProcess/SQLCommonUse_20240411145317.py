...
def count_records_in_table(table_name: str) -> str:
    # 建立数据库连接
    conn = pymysql.connect(**SQL_CONFIG)
    cursor = conn.cursor()
    ...
    try:
        ...
        if result:
            ...
            return f"表 '{table_name}' 中有 {record_count} 条数据。"
        ...
        # 发生错误时回滚
        conn.rollback()
        return f"查询表 '{table_name}' 数据时发生错误: {str(e)}"
...
result_message = count_records_in_table(table_name)
