...
def visualize_clusters(complete_trend_line: np.ndarray, cluster_labels: np.ndarray):
    ...
    # 绘制散点图，按照聚类标签进行着色
    plt.scatter(np.arange(len(complete_trend_line)), complete_trend_line, c=cluster_labels, cmap='viridis', alpha=0.1)
    ...
    for i in range(max(cluster_labels) + 1):
        ...
        plt.scatter(np.arange(len(complete_trend_line))[cluster_labels == i], complete_trend_line[cluster_labels == i], label=f'Cluster {i}', alpha=0.1)
...
# 使用示例
cluster_labels = cluster_analysis_with_guss(complete_trend_line, 3)
