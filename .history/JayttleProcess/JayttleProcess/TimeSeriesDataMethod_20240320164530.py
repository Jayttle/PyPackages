...
def plot_imfs_rm(imfs, rm):
    """绘制IMFs和残差项Rm"""
    num_imfs = len(imfs) + 1  # 增加一个子图用于绘制残差项

    # 创建子图布局
    fig, axes = plt.subplots(num_imfs, 2, figsize=(12, 3*num_imfs), sharex=True)
    
    # 绘制每个IMF的图形
    for i, imf in enumerate(imfs):
        axes[i, 0].plot(imf)
        axes[i, 0].set_ylabel(f"IMF {i+1}")

    # 绘制残差项的图形
    axes[-1, 0].plot(rm)
    axes[-1, 0].set_ylabel("Rm")
