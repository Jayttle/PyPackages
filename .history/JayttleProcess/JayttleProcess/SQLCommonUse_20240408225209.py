...
@log_function_call
def insert_data_default(func):
    def wrapper(*args, **kwargs):
        # 如果被装饰的函数是 insert_data，并且传入了参数，则执行原始的 insert_data 函数
        if func.__name__ == 'insert_data' and (args or kwargs):
            return func(*args, **kwargs)
        # 否则，执行默认的插入数据的 SQL 语句
        default_insert_data_query = """
        INSERT INTO met (Time, StationID, Temperature, Humidness, Pressure, WindSpeed, WindDirection)
        VALUES
        ('2024-04-08 12:00:00', 7, 25.5, 50.0, 1013.25, 10.0, 'N')
        """
...
def insert_data(func):
    def wrapper(*args, **kwargs):
        # 如果提供了两个参数且类型符合，则执行插入数据的 SQL 语句
        if len(args) == 2 and isinstance(args[0], int) and isinstance(args[1], str):
            ...
            insert_query = """
            INSERT INTO met (Time, StationID, Temperature, Humidness, Pressure, WindSpeed, WindDirection)
            VALUES (%s, %s, 25.5, 50.0, 1013.25, 10.0, 'N')
            """
...
# 调用函数演示两种用法
# insert_data()  # 执行默认的插入数据操作
insert_data(7, "Station A")  # 插入指定的数据
