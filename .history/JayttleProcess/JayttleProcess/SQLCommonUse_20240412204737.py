...
@log_function_call
def calculate_daily_missing_percentage_dict(
    start_time: datetime, 
    end_time: datetime, 
    missing_intervals: list[tuple[datetime, datetime, int]]
    ...
    返回：
    - dict[datetime.date, float]，每天的日期和缺失百分比
    """

    daily_missing_percentage = {}
    ...
    while current_day <= end_time:
        ...
        seconds_in_day = (day_end - day_start).total_seconds()

        missing_seconds_in_day = 0
        for interval_start, interval_end, missing_seconds in missing_intervals:
            ...
            if overlap_start <= overlap_end:
                ...
                missing_seconds_in_day += overlap_duration_seconds

        missing_percentage = (missing_seconds_in_day / seconds_in_day) * 100
        daily_missing_percentage[current_day.date()] = missing_percentage
        
        current_day += timedelta(days=1)

    return daily_missing_percentage
...
def calculate_daily_data_availability(min_time: datetime, max_time: datetime, missing_intervals: list[tuple[datetime, datetime, int]]) -> dict[datetime, float]:
    ...
    for date in date_range:
        ...
        # 将结果添加到字典
        daily_data_availability[start_of_day] = data_availability_percentage

    return daily_data_availability


@log_function_call
def export_missing_percentage_to_file(daily_missing_percentage: dict[datetime, float]) -> None:
    """
    将每日缺失百分比字典输出到文件
    
    参数：
        daily_missing_percentage (Dict[datetime, float]): 包含日期和每日缺失百分比的字典
        
    返回：
        None
    """
    # 将字典转换为 DataFrame
    df = pd.DataFrame(list(daily_missing_percentage.items()), columns=['Date', 'met_miss'])
...
@log_function_call
def export_data_availability_to_file(daily_data_availability: dict[datetime, float]) -> None:
    """
    将每日数据存有率字典输出到文件
    
    参数：
        daily_data_availability (Dict[datetime, float]): 包含日期和每日数据存有率的字典
        
    返回：
        None
    """
    # 将字典转换为 DataFrame
    df = pd.DataFrame(list(daily_data_availability.items()), columns=['Date', 'Data Availability (%)'])

    # 计算数据存有率（100减去每日缺失百分比）
    df['Data Availability (%)'] = 100 - df['Data Availability (%)']

    # 将 0 的数据存有率标识为 'N/A'
    df['Data Availability (%)'] = df['Data Availability (%)'].apply(lambda x: 'N/A' if x == 0 else x)
...
missing_intervals = load_missing_intervals(file_path)
...
# 示例用法
daily_missing_percentage = calculate_daily_missing_percentage_dict(min_time, max_time, missing_intervals)

# 计算SQL_accelerometer和SQL_tiltmeter的每日数据存有率
daily_data_availability_accelerometer = calculate_daily_data_availability(min_time, max_time, missing_intervals_accelerometer)
daily_data_availability_tiltmeter = calculate_daily_data_availability(min_time, max_time, missing_intervals_tiltmeter)

# 合并数据存有率字典
daily_data_availability_combined = {}
for date in daily_data_availability_accelerometer.keys():
    # 如果日期在SQL_accelerometer中
    if date in daily_data_availability_accelerometer:
        availability_accelerometer = daily_data_availability_accelerometer[date]
    else:
        availability_accelerometer = 100  # 如果没有缺失数据，默认数据存有率为100

    # 如果日期在SQL_tiltmeter中
    if date in daily_data_availability_tiltmeter:
        availability_tiltmeter = daily_data_availability_tiltmeter[date]
    else:
        availability_tiltmeter = 100  # 如果没有缺失数据，默认数据存有率为100

    # 计算平均数据存有率
    daily_data_availability_combined[date] = (availability_accelerometer + availability_tiltmeter) / 2
    

export_data_availability_to_file(daily_missing_percentage)
