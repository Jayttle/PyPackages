...
def count_files_per_day(rinex_files_info: list[RinexFileInfo]) -> dict[datetime.date, dict[str, int]]:
    ...
    返回：
        dict[date, dict[str, int]]: 每天文件数的字典，键为日期，值为另一个字典，
                                    其中键为标记站名，值为文件数。
    """
    # 初始化一个包含所有标记站名的列表
...
# 统计每天文件数
files_per_day = count_files_per_day(rinex_files_info)

output_file_path = "D:\\Ropeway\\GNSS\\files_per_day.txt"  # 输出文件路径

# 将结果写入到文件中
with open(output_file_path, 'w') as output_file:
    for key, values in files_per_day.items():
        output_file.write(f"{key}\n")  # 写入日期
        for marker_name, num in values.items():
            output_file.write(f"{marker_name}: {num}\n")  # 写入标记站名和对应文件数
        output_file.write("\n")  # 写入空行以分隔每个日期的数据