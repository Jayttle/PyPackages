...
def calculate_similarity(ts1, ts2, similarity_metric='euclidean'):
    ...
    Args:
        ...
        similarity_metric (str, optional): 相似性度量方法，默认为'euclidean'（欧氏距离）。可选值包括'euclidean'（欧氏距离），
                                            'pearson'（皮尔逊相关系数），'spearman'（Spearman等级相关系数）等。
    ...
    if similarity_metric == 'euclidean':
        ...
        # 计算Spearman等级相关系数
        similarity = np.corrcoef(ts1, ts2, rank=True)[0, 1]
    else:
...
clusters = time_series_clustering(ts_data, num_clusters)

# 打印每个时间序列的聚类标签
print("聚类结果:")
for i, cluster in enumerate(clusters):
    print(f"时间序列{i+1}的聚类标签: {cluster}")

ts1 = generate_random_data()
ts2 = generate_random_data()
ts1_value=[data.value for data in ts1]
ts2_value=[data.value for data in ts2]
similarity_metric = 'euclidean'  # 相似性度量方法为欧氏距离

similarity = calculate_similarity(ts1_value, ts2_value, similarity_metric)
print("时间序列之间的相似性（欧氏距离）:", similarity)
# 欧氏距离
euclidean_distance = calculate_similarity(ts1_value, ts2_value, 'euclidean')
print("欧氏距离:", euclidean_distance)

# 皮尔逊相关系数
pearson_correlation = calculate_similarity(ts1_value, ts2_value, 'pearson')
print("皮尔逊相关系数:", pearson_correlation)

# 斯皮尔曼等级相关系数
spearman_correlation = calculate_similarity(ts1_value, ts2_value, 'spearmans')
print("斯皮尔曼等级相关系数:", spearman_correlation)
