...
def create_time_series_data(values, datetimes):
    ...
    return time_series_data
def plot_TimeSeriesData(TimeSeriesData, isShow=False,SaveFilePath=None):
...
def perform_durbin_watson_test(residuals):
    """执行德宾沃森检验"""
    durbin_watson_statistic = stats.durbin_watson(residuals)
...
def fit_Ridge_polynomial_trend_diff_alphas(data, degree=13, alphas=[0.1, 1.0, 10.0]):
    """使用岭回归拟合多项式趋势线，并绘制不同 alpha 值下的系数路径"""
    x = np.array(get_x_values(data))
    y = np.array([point.value for point in data])
    ...
    for i in range(len(coefs[0])):
        ...
        plt.plot(np.log(alphas), coef_values, label=f'Coef {i}')
    ...
    # 返回拟合结果
    ridge = Ridge(alpha=1.0)  # 使用默认的 alpha 值重新拟合模型
    ridge.fit(X_poly, y)
    y_pred = ridge.predict(X_poly)
...
degree=11
alpha=1.0
fit_Ridge_polynomial_trend_diff_alphas(data,11)
