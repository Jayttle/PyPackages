...
def parse_tabulated_data(filepath):
    ...
    # 查找包含框架线的行，这些行使用 '╒', '╤', '╕', '├', '┼', '┤', '╞', '╪', '╡'
    frame_lines = [i for i, line in enumerate(lines) if '├' in line or '╞' in line or '┤' in line]

    # 解析表头
    header_line = lines[frame_lines[0]].strip().split('│')[1:-1]
    headers = [header.strip() for header in header_line]

    # 解析数据行
    data = []
    for i in range(frame_lines[0] + 1, len(lines)):
        if '├' in lines[i] or '╞' in lines[i]:  # 跳过其他框架线
            continue
        if '┤' in lines[i]:  # 结束数据行
            break
        row_data = lines[i].strip().split('│')[1:-1]
...
# 调用函数
file_path = 'D:\Program Files (x86)\Software\OneDrive\PyPackages\output_table.txt'
data = parse_tabulated_data(file_path)
print(data)