...
class MergeFiles:
    ...
    def merge_files(files: list[RinexFileInfo], merge_path: str, rewrite: bool = False) -> bool:
        ...
        # 构建文件列表字符串
        file_list = " ".join([os.path.abspath(f.file_info) for f in files])

        # 构建参数列表
        arguments = [
            r"D:\Program Files (x86)\Software\OneDrive\C#\WPF\Cableway.Download\Rinex\gfzrnx_2.1.0_win64.exe",
            "-finp",
            file_list,  # 将文件列表作为单独的元素传递给 -finp 选项
            "-kv",
            "-fout",
            merge_full_name,
            "-vo",
        ...
        process = subprocess.Popen(
            ["D:\Program Files (x86)\Software\OneDrive\C#\WPF\Cableway.Download\Rinex\gfzrnx_2.1.0_win64.exe", arguments],
            shell=False,
        ...
        if process.returncode == 0:
            ...
            print("External program encountered an error.")
