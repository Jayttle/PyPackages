...
def gzip_folder_path_with_winrar(folder_path: str = r"D:\Ropeway\GNSS\FTP\B011") -> None:
    """
    使用 WinRAR 解压指定文件夹中的所有 .Z 文件。
...
def find_8files():
    ...
    rinex_files_info: list[RinexFileInfo] = read_rinex_files_info(file_list_path)
        ...
        if os.path.isdir(directory_path):
            ...
            for file_name in os.listdir(directory_path):
                ...
                if file_name.endswith("_MO.rnx"):
                    ...
                    # 检查第四行数据的前四个字符是否等于文件名的前四个字符
                    if len(lines) >= 4 and lines[3][:4] != first_four_chars:
                        # 修改第四行数据的前四个字符为文件名的前四个字符
                        lines[3] = first_four_chars + lines[3][4:]
                        # 将修改后的内容写回文件中
