...
def crx_to_rnx(file_info: RinexFileInfo, rnx_path: Optional[str] = None, delete_crx_file: bool = True, overwrite_repeated: bool = False) -> Optional[RinexFileInfo]:
    ...
    if file_info.is_directory:
        return file_info
    if not file_info.file_path.lower().endswith('.crx'):
        return file_info

    rnx_str = "RNX" if "CRX" in file_info.file_path else "rnx"
    file_name_parts = file_info.file_path.split('.')
    file_name_parts[-1] = rnx_str
    new_file_name = ".".join(file_name_parts)

    rnx_path = rnx_path or file_info.directory_name
    new_full_name = os.path.join(rnx_path, os.path.basename(new_file_name))
    rnx_file = RinexFileInfo(new_full_name, False)

    if rnx_file.exists:
        if not overwrite_repeated:
            return rnx_file
        else:
            os.remove(rnx_file.file_path)
...
for item in ["R031", "R032", "R051", "R052", "R071", "R072", "R081", "R082"]:
    ...
    print(f"{item}:{len(dates_with_specific_count)}")
    # 示例用法
    output_file_path = f"D:\\Ropeway\\GNSS\\output_{item}.txt"
    marker_name = item
