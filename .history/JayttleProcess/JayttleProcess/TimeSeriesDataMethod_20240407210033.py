...
def elbow_method(complete_trend_line: np.ndarray, max_clusters: int):
    """
    使用肘部法则选择最优的聚类数
    参数：
        - complete_trend_line: 包含完整趋势线的数组
    ...
    # 绘制肘部法则图
    plt.plot(range(1, max_clusters + 1), distortions, marker='o', label='损失函数值')
    plt.xlabel('聚类数')
    plt.ylabel('损失函数值')
    plt.title('肘部法则')
    for i, txt in enumerate(distortions):
        plt.annotate(round(txt, 2), (i+1, distortions[i]), textcoords="offset points", xytext=(0,10), ha='center')
    plt.legend()
    plt.show()

def elbow_method_GMM(complete_trend_line: np.ndarray, max_clusters: int):
    """
    使用肘部法则选择最优的聚类数
    参数：
        - complete_trend_line: 包含完整趋势线的数组
...
num_clusters = 3
cluster_labels = cluster_analysis_GMM(value_np, num_clusters)
silhouette, davies_bouldin, calinski_harabasz = cluster_cluster_labels_with_score(value_np, cluster_labels)
