...
def visualize_gantt_chart(start_time: datetime, end_time: datetime,
                           data_intervals: list[tuple[datetime, datetime, int]],
                           record_intervals: list[tuple[datetime, datetime]]) -> None:
    # 计算甘特图的时间范围
    total_duration = end_time - start_time
    num_weeks = total_duration.days // 7
    end_of_last_week = start_time + timedelta(weeks=num_weeks)
    if end_of_last_week < end_time:
        num_weeks += 1
    ...
    # 绘制数据时间缺失段
    for start, end, _ in data_intervals:
        ax.barh(0, end - start, left=start, height=0.3, color='red', align='center', alpha=0.5)

    # 绘制记录段
    for start, end in record_intervals:
        ax.barh(0, end - start, left=start, height=0.3, color='green', align='center', alpha=0.5)
    ...
    # 设置标题和网格
    plt.title('Gantt Chart for Data Intervals and Record Intervals')
    plt.grid(True)
...
record_intervals = [
    (datetime(2023, 4, 13, 16, 4, 40, 985000), datetime(2023, 4, 30, 13, 5, 16, 701000))
]

# 可视化甘特图
visualize_gantt_chart(
    datetime(2023, 4, 13, 16, 4, 40, 985000),
    datetime(2023, 4, 30, 13, 5, 16, 701000),
    data_intervals,
    record_intervals
)