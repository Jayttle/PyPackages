...
def group_by_hour(file_list):
    key_func = lambda x: int(x.start_gps_time.strftime("%H"))
    groups = {}
    for key, group in itertools.groupby(sorted(file_list, key=key_func), key=key_func):
...
def proj_merge_rnx():
    ...
    # 输出分组结果
    for group_key, group_value in grouped_o_files.items():
        print(f'Group {group_key}')
        for file_info in group_value:
