...
def create_marker_name_excel(marker_name_files_per_day_1h: dict[datetime.date, dict], file_path: str = "marker_name_statistics.xlsx") -> None:
    # 创建一个新的工作簿
    wb = Workbook()
    ...
    # 写入 marker_name 标题行
    col_idx = 2
    for marker_name in marker_name_files_per_day_1h.values():
        ws_marker_name.cell(row=1, column=col_idx, value=str(next(iter(marker_name.keys()))))
        col_idx += 1

    # 写入数据
    for idx, (date_obj, files_per_day) in enumerate(marker_name_files_per_day_1h.items(), start=2):
        ws_marker_name.cell(row=idx, column=1, value=date_obj)
        for col_idx, count in enumerate(files_per_day.values(), start=2):
            ws_marker_name.cell(row=idx, column=col_idx, value=str(count))  # 将整数转换为字符串

    # Set alignment and adjust column widths as before
    # [Code for alignment and setting column widths]
...
for marker_name, files_per_day in marker_name_files_per_day_1h.items():
    print("Marker Name:", marker_name)
    for date, count in files_per_day.items():
        print("Date:", date, "Count:", count)


dt1 = datetime.date(2018, 1, 16)
