...
# 创建一个字典来存储每一天的数据持续时间
daily_duration = defaultdict(timedelta)

# 遍历 RinexFileInfo 列表，统计每一天的数据持续时间
for file_info in rinex_files_info:
    start_date = file_info.start_gps_time.date()
    end_date = (file_info.start_gps_time + file_info.duration).date()
    current_date = start_date
    while current_date <= end_date:
        daily_duration[current_date] += timedelta(days=1)
        current_date += timedelta(days=1)

# 计算每一天的总时长
total_duration_per_day = {date: timedelta(days=1) for date in daily_duration}

# 计算每一天数据存在的时长，并计算占有率
occupancy_rate_per_day = {date: daily_duration[date] / total_duration_per_day[date] for date in daily_duration}

# 输出结果
for date, occupancy_rate in occupancy_rate_per_day.items():
    print(f"On {date.strftime('%Y-%m-%d')}, data occupancy rate: {occupancy_rate:.2%}")
