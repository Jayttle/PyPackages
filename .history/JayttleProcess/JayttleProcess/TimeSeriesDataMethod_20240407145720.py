...
def calculate_cusum(time_series: List[TimeSeriesData], target: float) -> List[float]:
...
def plot_cusum_pos_and_neg(cusum_pos: List[float], cusum_neg: List[float]):
    plt.figure(figsize=(12, 6))
    plt.plot(cusum_pos, label='CUSUM Positive', color='blue')
    plt.plot(cusum_neg, label='CUSUM Negative', color='red')
    plt.axhline(y=0, color='k', linestyle='--')  # Target line
