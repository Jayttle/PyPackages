...
def insert_data_default(func):
    def wrapper(*args, **kwargs):
        # 如果被装饰的函数是 insert_data，并且传入了两个参数且类型符合，则执行原始的 insert_data 函数
        if func.__name__ == 'insert_data' and len(args) == 2 and isinstance(args[0], int) and isinstance(args[1], str):
