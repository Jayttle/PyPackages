...
def predict_analyze_evaluate(time_series, order):
    # 创建AR模型
    ar_model = sm.tsa.ar_model.AR(time_series).fit(maxlag=order[0])
    
    # 创建ARIMA模型
    arima_model = sm.tsa.ARIMA(time_series, order=order).fit()
    
    # 预测
    ar_predictions = ar_model.predict(start=len(time_series), end=len(time_series)+2)
    arima_predictions = arima_model.predict(start=len(time_series), end=len(time_series)+2)
    
    print("AR模型预测结果:", ar_predictions)
