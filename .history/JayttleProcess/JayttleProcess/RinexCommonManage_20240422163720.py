...
def proj_1(): 
    ...
    rinex_files_info = read_rinex_files_info_version2(file_list_path)
        # 打开一个文本文件以写入方式
    with open('rinex_files_info.txt', 'w') as file:
        # 遍历 rinex_files_info 列表中的每个 RinexFileInfo 对象
        for info in rinex_files_info:
            # 将每个 RinexFileInfo 对象的信息写入文件
            file.write(info.file_name + '\t' + str(info.compressed) + '\t + str(info.format) + '\n')
