...
def plot_polynomial_tread(data,trend_line):
    ...
    plt.title('时序数据拟合趋势线')
    plt.show()
def visualize_change_rate(change_rates):
    # 绘制图表
...
def detect_change_points(cusum_values, threshold):
    """
...
def detect_change_points(cusum_values, threshold, window_size):
    """
    滑动窗口来检测连续的CUSUM值
...
# 拟合多项式趋势线（这里选择三次多项式）
threshold = 1  # 设置阈值,根据具体情况调整
cusum_values = cusum(data, threshold)
plot_cusum(data,cusum_values)
for value in cusum_values:
    print(value)
change_points=detect_change_points(cusum_values,1,1)
