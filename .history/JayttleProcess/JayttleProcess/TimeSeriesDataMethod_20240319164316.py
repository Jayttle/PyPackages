...
def predict_analyze_evaluate(time_series, order):
    # 创建ARMA模型
    arma_model = sm.tsa.ARMA(time_series, order=order).fit()
    
    # 创建ARIMA模型
    arima_model = sm.tsa.ARIMA(time_series, order=order).fit()
    
    # 预测
    arma_predictions = arma_model.predict(start=len(time_series), end=len(time_series)+2)
    arima_predictions = arima_model.predict(start=len(time_series), end=len(time_series)+2)
    
    print("ARMA模型预测结果:", arma_predictions)
...
order = (1, 0)  # AR阶数为1，MA阶数为0
predict_analyze_evaluate(ts_data,order)
